...
"[project]/src/utils/htmlReader.tsx [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
 ...
 */ function readSnippetFile(snippetPath, currentFilePath) {
    try {
        // Resolve the snippet path relative to the current HTML file
        const currentDir = __TURBOPACK__imported__module__$5b$externals$5d2f$path__$5b$external$5d$__$28$path$2c$__cjs$29$__["default"].dirname(currentFilePath);
        const resolvedRelativePath = __TURBOPACK__imported__module__$5b$externals$5d2f$path__$5b$external$5d$__$28$path$2c$__cjs$29$__["default"].join(currentDir, snippetPath);
        const filePath = __TURBOPACK__imported__module__$5b$externals$5d2f$path__$5b$external$5d$__$28$path$2c$__cjs$29$__["default"].join(process.cwd(), 'public', resolvedRelativePath);
        ...
        // Read the file
        const snippetContent = __TURBOPACK__imported__module__$5b$externals$5d2f$fs__$5b$external$5d$__$28$fs$2c$__cjs$29$__["default"].readFileSync(filePath, 'utf-8');
        ...
        if (bodyMatch) {
            ...
            const snippetDir = __TURBOPACK__imported__module__$5b$externals$5d2f$path__$5b$external$5d$__$28$path$2c$__cjs$29$__["default"].dirname(resolvedRelativePath);
            content = content.replace(/<img([^>]*)\ssrc="([^"]+)"/g, (match, attrs, src)=>{
                ...
                const resolvedPath = __TURBOPACK__imported__module__$5b$externals$5d2f$path__$5b$external$5d$__$28$path$2c$__cjs$29$__["default"].join(snippetDir, src);
...
async function readHTMLFile(href) {
    try {
        // Remove leading slash
        const cleanHref = href.startsWith('/') ? href.slice(1) : href;
        const filePath = __TURBOPACK__imported__module__$5b$externals$5d2f$path__$5b$external$5d$__$28$path$2c$__cjs$29$__["default"].join(process.cwd(), 'public', cleanHref);
        // Check if file exists
        if (!__TURBOPACK__imported__module__$5b$externals$5d2f$fs__$5b$external$5d$__$28$fs$2c$__cjs$29$__["default"].existsSync(filePath)) {
        ...
        // Read the file
        const htmlContent = __TURBOPACK__imported__module__$5b$externals$5d2f$fs__$5b$external$5d$__$28$fs$2c$__cjs$29$__["default"].readFileSync(filePath, 'utf-8');
 ...
 */ async function parseHTMLContent(htmlContent, currentFilePath) {
 ...
 */ async function cleanHTMLContent(html, currentFilePath) {
    ...
    html = html.replace(/<MadCap:snippetText[^>]*\/>/g, '<span class="snippet-text">📄 Snippet</span>')// Handle MadCap conditional text - extract content and remove MadCap wrapper
    .replace(/<MadCap:conditionalText[^>]*>([\s\S]*?)<\/MadCap:conditionalText>/g, '$1')// Remove other MadCap elements
    .replace(/<MadCap:[^>]*>[\s\S]*?<\/MadCap:[^>]*>/g, '').replace(/<MadCap:[^>]*\/>/g, '');
    ...
    html = html// Fix broken link tags like <>text</></a> (after MadCap processing)
    .replace(/<>([^<]+)<\/><\/a>/g, '<span class="broken-link">$1</span>')// Convert name anchors to proper id spans
    .replace(/<a name="([^"]+)"[^>]*><\/a>/g, '<span id="$1" class="anchor"></span>')// Fix italic bold tags
    .replace(/<b style="font-style: italic;">([^<]+)<\/b>/g, '<em>$1</em>')// Convert i to em for better semantics
    .replace(/<i>([^<]+)<\/i>/g, '<em>$1</em>')// Keep b tags as they are for now to test
    // .replace(/<b>([\s\S]*?)<\/b>/g, '<strong>$1</strong>')
    // Convert u to proper underline styling
    .replace(/<u>([\s\S]*?)<\/u>/g, '<span style="text-decoration: underline;">$1</span>')// Fix broken closing tags
    .replace(/<\/a><b>/g, '</a> <strong>')// Add proper spacing around headings
    ...
    .replace(/<\/img>/g, '')// Convert relative image paths to absolute paths
    .replace(/<img([^>]*)\ssrc="([^"]+)"/g, (match, attrs, src)=>{
        ...
        const currentDir = __TURBOPACK__imported__module__$5b$externals$5d2f$path__$5b$external$5d$__$28$path$2c$__cjs$29$__["default"].dirname(currentFilePath);
        const resolvedPath = __TURBOPACK__imported__module__$5b$externals$5d2f$path__$5b$external$5d$__$28$path$2c$__cjs$29$__["default"].join(currentDir, src);
        const absolutePath = '/' + resolvedPath.replace(/\\/g, '/');
        return `<img${attrs} src="${absolutePath}"`;
    })// Add proper spacing around images (but not for inline images)
    .replace(/<img([^>]*)>/g, '<img$1>')// Add proper spacing around code blocks
    .replace(/<\/code>/g, '</code>').replace(/<code([^>]*)>/g, '<code$1>')// Add proper spacing around pre blocks
    .replace(/<\/pre>/g, '</pre>\n\n').replace(/<pre([^>]*)>/g, '<pre$1>\n')// Enhance code block containers with syntax highlighting classes
    .replace(/<div><pre><code>/g, '<div class="code-block syntax-highlighted"><pre><code class="language-text">').replace(/<\/code><\/pre><\/div>/g, '</code></pre></div>')// Add syntax highlighting classes to inline code blocks
    .replace(/<div class="code-block-inline"><code>/g, '<div class="code-block-inline"><code class="inline-code syntax-highlighted">')// Auto-detect and add language classes to code blocks
    ...
    .replace(/<code>([^<]*)<\/code>/g, '<code class="inline-code syntax-highlighted">$1</code>')// Fix code blocks that are not properly wrapped
    .replace(/<pre><code class="language-text">([^<]*)<\/code><\/pre>/g, '<div class="code-block syntax-highlighted"><pre><code class="language-text">$1</code></pre></div>')// Fix code blocks that are missing the wrapper div
    .replace(/<pre><code>([^<]*)<\/code><\/pre>/g, '<div class="code-block syntax-highlighted"><pre><code class="language-text">$1</code></pre></div>')// Fix code blocks that have inline-code class but should be code blocks - handle multiline content
    .replace(/<div><pre>\s*<code class="inline-code syntax-highlighted">([\s\S]*?)<\/code>\s*<\/pre>\s*<\/div>/g, '<div class="code-block syntax-highlighted"><pre><code class="language-text">$1</code></pre></div>')// Fix code blocks that are missing the proper wrapper - handle multiline content
    .replace(/<div><pre>\s*<code class="inline-code syntax-highlighted">([\s\S]*?)<\/code>\s*<\/pre>\s*<\/div>/g, '<div class="code-block syntax-highlighted"><pre><code class="language-text">$1</code></pre></div>')// Enhance code blocks within list items - remove br tags and clean up spacing
    .replace(/<li><strong>([^<]*)<\/strong><br \/>\s*<div><code>/g, '<li><strong>$1</strong><div class="code-block-inline"><code>').replace(/<\/code>\s*<\/div>\s*<\/li>/g, '</code></div></li>')// Remove any remaining br tags before code blocks in list items
    .replace(/<li><strong>([^<]*)<\/strong>\s*<br \/>\s*<div><code>/g, '<li><strong>$1</strong><div class="code-block-inline"><code>')// Handle direct code tags after br (without div wrapper)
    .replace(/<li><strong>([^<]*)<\/strong>\s*<br \/>\s*<code>/g, '<li><strong>$1</strong><div class="code-block-inline"><code>').replace(/<\/code>\s*<\/li>/g, '</code></div></li>')// Add proper spacing around blockquotes
    .replace(/<\/blockquote>/g, '</blockquote>\n\n').replace(/<blockquote([^>]*)>/g, '<blockquote$1>\n')// Handle line breaks and horizontal rules
    .replace(/<br\s*\/?>/g, '<br />').replace(/<hr\s*\/?>/g, '<hr />')// Add proper spacing around horizontal rules
    .replace(/<hr\s*\/?>/g, '\n<hr />\n\n')// Handle iframe elements - convert to responsive video containers
    .replace(/<iframe([^>]*)\s*\/?>/g, '<div class="video-container"><iframe$1></iframe></div>')// Clean up any duplicate iframe tags that might have been created
    .replace(/<\/iframe><\/iframe>/g, '</iframe>')// Clean up any orphaned iframe closing tags
    .replace(/<\/iframe>/g, '')// Handle table elements with proper spacing
    .replace(/<\/table>/g, '</table>\n\n').replace(/<\/thead>/g, '</thead>\n').replace(/<\/tbody>/g, '</tbody>\n').replace(/<\/tfoot>/g, '</tfoot>\n').replace(/<\/tr>/g, '</tr>\n').replace(/<\/th>/g, '</th>').replace(/<\/td>/g, '</td>')// Handle form elements
    .replace(/<\/form>/g, '</form>\n\n').replace(/<\/fieldset>/g, '</fieldset>\n\n').replace(/<\/legend>/g, '</legend>\n').replace(/<\/label>/g, '</label>').replace(/<\/input>/g, '').replace(/<\/textarea>/g, '</textarea>\n').replace(/<\/select>/g, '</select>\n').replace(/<\/option>/g, '</option>').replace(/<\/button>/g, '</button>\n')// Handle semantic elements
    ...
    .replace(/<\/cite>/g, '</cite>')// Handle time
    .replace(/<\/time>/g, '</time>')// Handle mark
    .replace(/<\/mark>/g, '</mark>')// Handle small
    .replace(/<\/small>/g, '</small>')// Handle sub and sup
    .replace(/<\/sub>/g, '</sub>').replace(/<\/sup>/g, '</sup>')// Handle kbd
    .replace(/<\/kbd>/g, '</kbd>')// Handle samp
    .replace(/<\/samp>/g, '</samp>')// Handle var
    .replace(/<\/var>/g, '</var>')// Handle abbr
    .replace(/<\/abbr>/g, '</abbr>')// Handle data
    .replace(/<\/data>/g, '</data>')// Handle output
    .replace(/<\/output>/g, '</output>')// Handle progress
...
var __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$utils$2f$htmlReader$2e$tsx__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/src/utils/htmlReader.tsx [app-route] (ecmascript)");
;
;
async function GET(request) {
    try {
        ...
        const content = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$utils$2f$htmlReader$2e$tsx__$5b$app$2d$route$5d$__$28$ecmascript$29$__["readHTMLFile"])(href);
        if (!content) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
...
//# sourceMappingURL=%5Broot-of-the-server%5D__f909744b._.js.map